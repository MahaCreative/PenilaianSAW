  ...
  const fetchHistory = async (query) => {
    ...
    try {
      const response = await axios.get(
        GlobalUrl +
          `/api/guru/history-penilaian-siswa/${query.periode_id}?nama_siswa=${query.nama_siswa}&nama_guru=${query.nama_guru}`,
        {
          headers: {
            Authorization: `Bearer ${useToken}`,
          },
        ...
        <View>
          <View className="w-full px-2 py-1">
            ...
            <View className="flex flex-row  py-1 gap-x-1 items-center justify-between w-full  ">
              <View className="bg-pink-500 rounded-md py-1 px-2 text-white w-1/2 ">
                <View className="flex items-center">
              ...
              <View className="bg-blue-500 rounded-md py-1 px-2 text-white w-1/2 ">
                <View className="flex items-center">
                  <FontAwesome6 name="ranking-star" size={32} color="white" />
                  <Text className="text-xs text-white font-light">
                    Rangking 3
